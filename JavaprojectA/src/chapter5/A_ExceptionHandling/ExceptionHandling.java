...
// 예상치 못한 값을 받아 처리할떼 발생

// 시스템 오류 : 개발자가 코드상으로 조치를 취할 수 없는 문제 (전원 부족, 메모리 부족)
// 컴파일 에러 : 컴파일이 불가능한 상태 (오타, 참조타입)
// 런타입 에러 : 프로그램이 실행 도중 발생하는 문제 (빌드 실패, 포트 중복, 데이터베이스 연결 실패)
// 로직 에러 : 프로그램의 아웃풋이 예상과 다르게 나오는 문제
// 예외 : 정상적인 입력에서는 정상처리가 되지만 비정상적인 입력에서는 처리가 되지 않는 문제
// 예시)null 값 참조, 로직 실행 중 데이터베이스 연결 실패
...
//		System.out.println(numbers[4]);
		...
		// catch구문에 해당 에외 상황에 대한 대처 구문을 작성
		
		// 사용 방법
		// try { 예외가 발생할 수 있는 문장 }
		// catch ( 예외타입 예외참조변수 ) { 예외가 발생했을때 처리할 문장 }
